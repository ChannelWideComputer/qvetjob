...
class Model_service extends CI_Model {
	...
	{ 
			...
			$this->db->start_cache();
			$this->db->join('tb_category', 'tb_category.category_ID = tb_news.category_ID', 'left');
			$this->db->where('news_Type',$this->type);
			//$this->db->where('tb_category.category_Status','1');
			$this->db->group_start();
			$this->db->like('news_Name_TH', $word)->or_like('news_Name_TH', $word);
	...
	{ 
	    if($home== "home"){
			...
			$data = $this->db->where('news', 1)->get('tb_news')->result_array();*/
			$this->db->where('news_Home', 1);
			...
			$page_range = 5;
			$qry_string = "";
			$page_url = base_url('services/index/'.$home);
			...
			$this->db->start_cache();
			$this->db->join('tb_category', 'tb_category.category_ID = tb_news.category_ID', 'left');
			$this->db->where('tb_news.news_Type',$this->type );

			if(!empty($home))
			{
			$this->db->where('tb_news.category_ID',$home);
			}
			$this->db->group_start();
			$this->db->like('news_Name_TH', $word)->or_like('news_Name_TH', $word);
			$this->db->or_like('category_Name_TH', $word)->or_like('category_Name_EN', $word);
			...
			if(!empty($c)){
			    $this->db->where('tb_news.category_ID', $c);
			}
			$this->db->order_by('category_Sort', 'ASC');
			$this->db->order_by('news_Sort', 'ASC');
			$this->db->stop_cache();
			...
			$this->db->limit($rows_per_page, $start_row);
			$query = $this->db->where('tb_news.news_Type= "service"')->order_by('news_Sort', 'ASC')->get('tb_news');
			$this->db->flush_cache();
	...
	{ 
			...
			$query = $this->db->where('news_Type', $this->type)->where('category_ID', $_POST["category_ID"] )->get('tb_news');
			$Num_Rows = $query->num_rows();
			$Sort = $Num_Rows+1;
...
if (!empty($_POST["gallery_ID"])) {
	...
	{ 
		...
		if($this->input->post('id')!=""){
			...
			$query = $this->db->get_where('tb_news', array('news_ID' => $id, 'category_ID' =>$_POST['data_id'] ));
			$row = $query->row_array();
	...
	{ 
			...
			if($new_sort > $old_sort){ 
				...
				$this->db->set('news_Sort', 'news_Sort-1', FALSE);
				$this->db->where("news_Sort Between '$old_sort' and '$new_sort' AND category_ID = '$category_ID'  AND news_ID != '$id' ");
				$this->db->update('tb_news');
			
			}else{
				
				$this->db->set('news_Sort', 'news_Sort+1', FALSE);
				$this->db->where("news_Sort Between '$new_sort' and '$old_sort' AND category_ID = '$category_ID'  AND news_ID != '$id'");
				$this->db->update('tb_news');
