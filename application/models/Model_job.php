...
class Model_job extends CI_Model {
	...
	{ 
		...
		if ($_FILES["news_Images2"]["name"] != "") {


			$rename = "PHOTO_news_" . date("d-m-Y_Hms");


			$handle = new upload($_FILES["news_Images2"]);
	...
	{ 
		...
		if (!empty($_FILES["news_Images2"]["name"])) {
			...
			if ($handle->processed) {
				...
				@unlink("assets/upload/news/$_POST[news_Images2]");
			}
