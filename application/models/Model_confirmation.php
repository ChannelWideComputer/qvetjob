...
class Model_confirmation extends CI_Model {
	...
	{ 
		...
		$current_page = 1;
		$rows_per_page = 12;
		$page_url = base_url('confirmation');
		$page_range = 5;
		$qry_string = "";
		$pagetype = 0;
		...
		//$this->db->join('tb_sales_status','tb_sales_status.sales_id=tb_sales_statussales_pay_status');
		$this->db->group_start();
		...
		$query2 = $this->db->get('tb_sales_status');
		if($Num_Rows > 0 ) {
			$data["status"] = $query2->result_array();
		}
		...
		foreach ($query->result_array() as $row){
			...
			foreach ($query2->result_array() as $row2){
				...
				$data["sales"]["sales_detail"][] = array(
					...
					'product_Name_EN' => $row2["product_Name_EN"],
