...
class Model_category extends CI_Model {
	...
	{ 
		...
		if (@$_FILES["category_Images"]["name"] != "") {
			...
			if ($handle->processed) {
				...
				unlink("assets/upload/product/$_POST[category_Images]");
			}
	...
	{ 
		...
		if($new_sort > $old_sort){ 
			...
			$this->db->set('category_Sort', 'category_Sort-1', FALSE);
			$this->db->where("category_Sort Between '$old_sort' and '$new_sort' AND type = '$type' AND category_ID != '$id' ");
			$this->db->update('tb_category');
			
		}else{

			$this->db->set('category_Sort', 'category_Sort+1', FALSE);
			$this->db->where("category_Sort Between '$new_sort' and '$old_sort' AND type = '$type' AND category_ID != '$id' ");
			$this->db->update('tb_category');
